...
const financialBoardSchema = new mongoose.Schema({
  perfilID: {
  ...
  icono: {
    type: String,
    trim: true
    // Icono del tablero (puede ser sugerido por IA basado en el nombre)
  },
  imagen: {
    type: String,
    trim: true
    // Imagen del tablero (opcional)
  },
  color: {
    type: String,
    default: '#3B82F6',
    trim: true
    // Color primario del tablero (hex, default: azul)
  },
  moneda: {
    type: String,
    trim: true,
...
financialBoardSchema.pre('save', function(next) {
  this.saldo = this.ingresos - this.gastos;
  next();
});

// Método para actualizar ingresos y gastos desde transacciones
financialBoardSchema.methods.recalcularTotales = async function() {
  const Transaction = require('@models/Transaction');
  const Income = require('@models/Income');
  
  // Calcular ingresos desde Income
  const ingresos = await Income.find({ tableroID: this._id });
...
financialBoardSchema.index({ perfilID: 1, id_mes: 1, año: 1 });
financialBoardSchema.index({ perfilID: 1, mes: 1, año: 1 });

module.exports = mongoose.model('FinancialBoard', financialBoardSchema);
